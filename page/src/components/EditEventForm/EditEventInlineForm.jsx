...
const EditEventInlineForm = ({ event, onClose }) => {
  ...
  const validateForm = () => {
    ...
    if (formData.hasSponsoring) {
      if (!formData.sponsoringUrl.trim()) newErrors.sponsoringUrl = t('addEvent.errors.sponsoringUrlRequired');
      else { try { new URL(formData.sponsoringUrl); } catch { newErrors.sponsoringUrl = t('addEvent.errors.sponsoringUrlInvalid'); } }
    }
  ...
  const generateIssueBody = () => {
    ...
    compare(
      'Estimated Attendees',
      event.attendees ? String(event.attendees) : '',
      formData.attendees
    );
